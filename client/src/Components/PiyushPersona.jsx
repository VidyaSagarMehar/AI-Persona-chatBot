...
function PiyushPersona() {
	...
	const [messages, setMessages] = useState([
		{
			role: 'assistant',
			content: 'Hello, to aaj hmlog kis topic par bat karne wale hai?',
		},
	]);
	...
	const sendMessage = async () => {
		...
		try {
			const res = await fetch('http://localhost:5000/chat/piyush', {
				method: 'POST',
			...
			<div className="relative z-10 bg-slate-900/80 backdrop-blur-sm border-b border-slate-700/50">
				<div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
					...
					<div className="flex items-center gap-3">
						...
						<div>
							<h1 className="text-xl font-bold text-white">Piyush Sir</h1>
							<p className="text-sm text-purple-400">Tech Mentor & Guide</p>
						</div>
					</div>
